...
def show_camera_axes(viz, model, data, q_current, camera_link_name="camera_link", axis_length=0.15, set_camera_view=True):
    ...
    if set_camera_view:
        ...
        forward_local = np.array([0.0, 0.0, 1.0])  # 相机本地坐标系 z 轴
        up_local = np.array([0.0, -1.0, 0.0])      # 相机本地坐标系 -y 轴
        
        # 可选：添加向下倾斜角度来更好地观察机器人
        tilt_angle =0 # -30° 向下倾斜，可以调整这个值
        if tilt_angle != 0:
            # 绕相机 x 轴旋转（俯仰）
        ...
        # 计算相机看向的目标点（沿前向方向的一个点）
        lookat_distance = 5.0  # 可以调整这个距离
        lookat = position + forward_world * lookat_distance
        ...
        # 偏移距离（沿 up_world 方向移动多少米）
        offset_distance = 0.1

        # 新位置 = 原始位置 + 上方向 * 偏移距离
        new_position = position + up_world * offset_distance

        # # 方法1：使用 Meshcat 的相机控制接口
        # try:
        # 设置相机位置和方向
        viz.viewer["/Cameras/default"].set_property("position",new_position.tolist())
        viz.viewer["/Cameras/default/rotated/<object>"].set_property("position", [0, 0, 0])
        # viz.viewer["/Cameras/default/rotated/<object>"].set_property("lup_world", (lookat - position).tolist())
        viz.viewer["/Cameras/default/rotated/<object>"].set_property("lookAt", up_world.tolist())
                   
        # 在头部相机前方放置一个黄色球
        ball_distance = 0.2  # 球距离相机的距离
        ball_position = position + up_world * ball_distance
...
def main():
    ...
    csv_file_path = "/home/ryan/Documents/GitHub/AirExo-2-test/train_vedio/hand_landmarks_3d_offline.csv"
    ...
    show_camera_axes(viz, model, data, q_current, camera_link_name="camera_link", axis_length=0.15)

...
def run_dual_arm_trajectory(
    ...
    for i in range(N):
        ...
        fix = pin.AngleAxis(np.pi, np.array([0,0,1])).matrix() #fix the reverse coordinate
        l_world_R = cam_T.rotation @ l_local_R @ fix

        l_world_eul = pin.rpy.matrixToRpy(l_world_R)

        # 后续帧保持原有平滑约束
        q_sel_r, err_r = solve_one_frame(
            model, data, q, right_idxs, r_world_p, r_world_eul, right_end_effector,
            ik_attempts, noise_level, per_joint_max_delta, total_max_delta, lambda_weight
        )
        q[right_idxs] = q_sel_r[right_idxs]

        q_sel_l, err_l = solve_one_frame(
            model, data, q, left_idxs, l_world_p, l_world_eul, left_end_effector,
            ik_attempts, noise_level, per_joint_max_delta, total_max_delta, lambda_weight
        )
        q[left_idxs] = q_sel_l[left_idxs]

        print(f"Frame {i}: 右臂误差: {err_r:.4f}, 左臂误差: {err_l:.4f}")
        print(f"右臂关节角: {np.round(q[right_idxs], 2)}")
        print(f"左臂关节角: {np.round(q[left_idxs], 2)}")

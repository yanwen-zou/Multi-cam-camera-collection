"""
Dual-arm IK utilities.

功能：
- 读取 arm1/arm2 CSV 文件(列包含 px/py/pz + 四元数 qw/qx/qy/qz)
- 在IK过程中保存每一帧joint结果到 joint_data/solved_joint_trajectory2.csv
- 时间戳从0开始，1秒60帧
- 使用手眼 四元数和平移 作为 base_from_camera
"""
...
def read_dualarm_csv(right_csv: str, left_csv: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    # 直接根据文件内部列名解析 (无论文件名如何)
    # 只要保证CSV里确实写了 arm1_ 或 arm2_ 前缀一致即可
    # 如果右臂CSV里是 arm1_ 开头，也会按 arm1_ 取数。
    def _parse(df):
...
def solve_one_frame(
    model, data, q_curr, active_idxs, pos, eul, end_eff,
...
def run_dual_arm_trajectory(
    ...
    qw:float=0.17830153820333408, qx:float=-0.9165103818132152, qy:float=-0.06978412222066299,
    qz:float=-0.35120856736248757, tx:float=-0.46234216236290954, ty:float=0.09033619204505915, tz:float=0.495407108937691,
    ik_attempts:int=20, noise_level:float=0.01, per_joint_max_delta:float=0.15, total_max_delta:float=0.4,
    lambda_weight:float=0.1, sleep_each:float=0.004, package_dirs:Sequence[str]|None=None, set_initial_camera:bool=True):
    ...
    R_bc = quaternion_to_rotation_matrix(qw,qx,qy,qz)
    t_bc = np.array([tx,ty,tz],float)
    all_rows=[];dt=1.0/60.0

    for i in range(N):
        print(f"\n--- Frame {i} ---")
        r_w_p = R_bc @ r_pos[i] + t_bc
        l_w_p = R_bc @ l_pos[i] + t_bc
        r_w_R = R_bc @ pin.rpy.rpyToMatrix(r_eul[i])
        l_w_R = R_bc @ pin.rpy.rpyToMatrix(l_eul[i])
